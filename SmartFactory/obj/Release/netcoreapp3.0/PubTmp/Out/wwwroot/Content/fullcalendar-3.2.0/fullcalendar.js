        ...
        fgSegHtml: function(seg, disableResizing) {
            ...
            if (seg.isStart) {
                ...
                if (timeText) {
                    ...
                    //내가 추가한부분
                    var qw = htmlEscape(timeText);
                    var tt = "";
                    var who_style = "";

                    qw = qw.replace("a", "").replace("p", "");
                    ...
                    //timeHtml = '<span class="fc-time"  >' + tt + ' ' + qw + '</span>';


                    var check_time = tt + qw;
                    if (check_time.indexOf("오전12시") == -1) {
                        timeHtml = '';
            ...
            return '<a class="' + classes.join(' ') + '"' +
                ...
                '>' +
                '<div rel="tooltip" class="fc-content"   style="' + who_style + '"  title="[' + event.who + '] ' + '  ' + event.title + ' ">' +
                (this.isRTL ?
                    timeHtml + ' ' + titleHtml : timeHtml + '  ' + titleHtml   // 시간 타이틀 순서
                    //
                ) +
                '</div>' +
...
// 수정 이동 메인
	...
	------------------------------------------------------------------------------------------------------------------*/
        ...
        computePrevDate: function(date) {     
            ...
            var _nm = $("#now_month").val();
            var m = _nm - 1;

            $("#now_month").attr("value", m);

            var _open = $("#project_idx").val();
            var src = "/monitor/memo_list?_month=" + m + "&open=" + _open;   
       
       
            $("#external-events").attr("src", src);

            $("#external-events").load(src);      
	...
	------------------------------------------------------------------------------------------------------------------*/
        ...
        buildGotoAnchorHtml: function(gotoOptions, attrs, innerHtml) {
            ...
            finalOptions = {
// for serialization into the link 수정
    ...
    function EventManager() { // assumed to be a calendar
        ...
        function filterEventsWithinRange(events) {
            ...
            for (i = 0; i < events.length; i++) {
                ...
                if (event.start.clone().stripZone() < rangeEnd &&
